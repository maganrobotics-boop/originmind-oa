...
async function fixture(run) {
  ...
  try {
    for (const name of ['0002_ai_workbench.sql', '0003_ai_workbench_artifacts.sql']) await cp(new URL(name, directory), join(dir, name));
...
test('only the exact reviewed task migration bytes are accepted', async () => fixture(async dir => {
  const sql = await reviewedTaskSql(dir); assert.equal(Object.keys(expectedTaskSchema(sql)).length, 5);
...
test('empty or compatible partial task schemas may be initialized, but cannot pass the after gate', async () => fixture(async dir => {
  ...
  assert.equal(checkTaskSchema('before', payload([]), definitions).missing.length, 5);
