...
const validSecrets = {
  ...
  PUBLIC_LAB_AI_SERVICE_TOKEN: "A".repeat(42) + "_",
};
...
test("staging Worker secrets are accepted only from a private exact JSON file outside the worktree", async () => {
  ...
  try {
    ...
    for (const invalidToken of ["A".repeat(42), `${"A".repeat(42)}=`, `${"A".repeat(42)}+`]) {
      await writeFile(path, `${JSON.stringify({ ...validSecrets, PUBLIC_LAB_AI_SERVICE_TOKEN: invalidToken })}\n`, { mode: 0o600 });
      await assert.rejects(execute(process.execPath, [validator, path]), /exactly 43 unpadded base64url/u);
    }
...
test("dashboard-configured staging secrets expose only the exact required names", async () => {
  ...
  assert.throws(() => validateSecretList(JSON.stringify([
    { name: "FEISHU_LOGIN_APP_SECRET" },
