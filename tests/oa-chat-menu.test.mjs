...
test('every workspace header opens chat on the right and centers the current secondary title', async () => {
  ...
  assert.match(page, /<OaConversationTitle><div className="oa-topbar-secondary-title"><strong>\{secondaryTitle\}<\/strong>\{activeView === "knowledge" && knowledgeTab === "ask" && <OaChatStatus \/>\}<\/div><\/OaConversationTitle>/u);
