...
export function AdminMeetingMinutesLink() {
  ...
  return allowed ? <a className="oa-document-text-button" href="/admin/meeting-minutes">成员会议纪要</a> : null;
}
...
export function AdminMeetingMinutes() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (disposed) return;
        if (!Array.isArray(data.items) || !Number.isFinite(data.checkedAt) || (data.nextCursor !== null && typeof data.nextCursor !== 'string')) throw new Error('未取得有效列表，请重新核对。');
        ...
        if (selected) {
          try {
            ...
            if (disposed) return;
            if (!response.item || response.item.id !== selected || typeof response.item.material !== 'string' || typeof response.item.result !== 'string') throw new Error('未取得完整会议纪要。');
  ...
  async function download(format: 'source' | 'md' | 'docx') {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        if ([401, 403].includes(response.status)) { setDenied(true); setItems([]); setDetail(null); setCheckedAt(0); setError(data.error || '访问权限已变化，请重新登录。'); }
        throw new Error(data.error || '文件暂不可下载。');
      }
  ...
  return <main className="oa-meeting-admin">
    <header><a href="/">返回 OA</a><h1>成员会议纪要</h1><p>成员提交“会议纪要”任务后即可查看，不必等待 AI 处理完成或归档审批。未发送的本地附件不在此列表。</p><p>仅管理员可跨成员查看；不开放普通聊天、私聊或其他文档。查看不等于归档，知识库入库仍走原审批。</p></header>
