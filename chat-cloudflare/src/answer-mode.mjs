...
export function answerLengthInstruction(question) {
  ...
  if (answerMode(question) === 'deep') return '这是复杂任务：优先完整性，通常控制在 700–1000 个中文字；除非用户明确要求更长，不超过 1000 个中文字。';
  return '默认回答更完整一些，通常控制在 500–900 个中文字；简单事实问题可接近 300–500 字，技术、成果、合作边界等问题使用 600–1000 字；除非用户明确要求更长，不超过 1000 个中文字。';
}
