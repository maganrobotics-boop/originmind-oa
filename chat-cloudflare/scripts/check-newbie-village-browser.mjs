...
const server = createServer(async (request, response) => {
  try {
    ...
    const requestedPath = pathname === "/newbie-village" ? "/newbie-village.html" : pathname;
...
function dashboard(firstStatus = "not_started", displayName = "student") {
  return {
  ...
  for (const [name, viewport] of [["desktop", { width: 1280, height: 900 }], ["mobile", { width: 390, height: 844 }]]) {
    ...
    await page.route("**/api/**", async (route) => {
      ...
      if (pathname === "/api/newbie/dashboard") {
        await route.fulfill(signedIn ? { json: dashboard(taskStatus, displayName) } : { status: 401, json: { error: "请先登录。" } });
        ...
        await route.fulfill({ json: { saved: true, ...dashboard(taskStatus, displayName) } });
