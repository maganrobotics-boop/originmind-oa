...
OA AI 助手中的“会议模式”采用纯聊天流程：管理员发送 `@会议模式919700881`（九位会议号）即请求机器人入会，不显示会前表单；机器人入会后轮询飞书会中事件，展示全文转写并支持决策、待办、风险和重要内容标记。会中发送 `生成会议纪要` 可生成截至当前的全文和动态纪要而不中断记录；发送 `@结束会议` 或检测到飞书会议结束后，机器人退出并生成最终全文和纪要，随后自动提交 OA 管理员审批，只有审批状态为 `active` 才显示为已归档。

会议中显示“会议驾驶舱”：用户可通过浏览器 `getDisplayMedia` 主动授权显示飞书会议窗口，画面仅在本地 `<video>` 中播放，不上传、不录制；底部显示最新一行字幕，右侧显示最近讨论、人工标记和从字幕中白名单识别的飞书文档链接。输入 `@会议` 时默认补全为 `@会议模式919700881`。

仍不包含：飞书呼叫自动接听、`vc.bot.meeting_invited_v1` 事件回调、录音文件下载、语音发言，以及未经管理员审批自动发布纪要。关闭 OA 页面不会让机器人退出。
