...
export const metadata: Metadata = {
  ...
  description: "了解联合研发 OA 的用途、项目协作规则，以及成果、采购、保密协议、劳务申请和实验室知识助手的操作方法。",
};
...
const scenarios = [
  ...
  { id: "knowledge", icon: Bot, title: "分享或查询实验室知识", entry: "实验室 AI", text: "成员投稿，项目负责人或 OA 管理员审核入库；问答时显示引用依据。" },
];
  ...
  return <div className="oa-guide" id="top">
    ...
    <main id="guide-main" className="guide-main">
      ...
      <section className="guide-section" id="tasks" aria-labelledby="tasks-title">
        ...
        <article className="guide-task" id="knowledge"><h3>实验室 AI：把可靠经验沉淀为可引用知识</h3><ol><li>进入“实验室 AI”，先提问，确认知识库里是否已有相同内容。</li><li>需要补充时选择“提交知识”，写清标题、分类、摘要、完整正文；有原始记录时可附来源名称和 HTTP/HTTPS 链接。</li><li>提交后在“我的提交”查看状态。被退回时按审核意见修改并重新提交；被拒绝的版本不会进入知识库。</li><li>项目负责人或 OA 管理员核对准确性、适用范围和敏感信息后决定是否入库。只有“已入库”且未撤销的版本会用于知识问答，回答会附引用条目和原文片段。</li></ol><p className="guide-note">不要提交账号密码、访问密钥或不应在项目内部共享的个人隐私。知识助手的回答用于协作参考，关键操作仍应回看引用原文并按负责人要求执行。</p></article>
      </section>
      ...
      <section className="guide-section" id="faq" aria-labelledby="faq-title">
        ...
        <div className="guide-faq">
          ...
          <article><h3>为什么我提交的知识还不能被问到？</h3><p>新投稿需要先由项目负责人或 OA 管理员审核。请到“实验室 AI—我的提交”查看状态；只有显示“已入库”且未被撤销的当前版本会参与问答。</p></article>
