...
export async function GET(request: Request) {
  try {
    ...
    return fileJson({ lifecycle: { state: retention?.state || 'legacy', expiresAt: retention?.state === 'temporary' ? Math.max(retention.expires_at || 0, task.updated_at + TEMPORARY_FILE_TTL) : null,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { id, confirmed } = parsed.value;
    if (Object.keys(parsed.value).some(key => !['id', 'confirmed'].includes(key)) || typeof id !== 'string' || !UUID.test(id) || confirmed !== true) return fileJson({ error: '请确认核对过归档正文和脱敏情况。' }, 400);
    const task = await readTask(ctx.db, ctx.actor, id);
    if (!task) return fileJson({ error: '成果不存在、已清理或无访问权限。' }, 404);
    ...
    const pinned = await pinFileArchive(ctx.db, ctx.actor, id);
  ...
  } catch { return fileJson({ error: '归档结果尚未确认，请核对后重试；不会绕过审批或自动公开。' }, 503); }
}
