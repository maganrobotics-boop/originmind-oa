...
async function measureWaiting<T>(
  ...
  try {
    ...
    timings[stage] = elapsedMilliseconds(startedAt);
  }
}
...
export async function POST(request: Request) {
  ...
  if (!authorized.memberId || !authorized.accountUserId || !authorized.memberMutationRevision) {
    return finish(privateJson({ error: "知识问答仅向已激活、实名绑定的 OA 成员开放。" }, { status: 403 }));
  }
  if (!isJsonRequest(request)) return finish(privateJson({ error: "知识问答必须使用 JSON 格式提交。" }, { status: 415 }));
  const parsedBody = await readBoundedJsonObject(request, MAX_ASK_REQUEST_BYTES);
  if (!parsedBody.ok) return finish(privateJson(
  ...
  if (Object.keys(parsedBody.value).some((key) => key !== "question") || typeof parsedBody.value.question !== "string") {
    return finish(privateJson({ error: "问题格式不正确。" }, { status: 400 }));
  }
  if (!isWellFormedUnicode(parsedBody.value.question)) return finish(privateJson({ error: "问题包含无效的 Unicode 字符。" }, { status: 400 }));
  ...
  if (question.length < 2 || question.length > MAX_KNOWLEDGE_QUESTION_LENGTH) {
    return finish(privateJson({ error: `问题需为 2–${MAX_KNOWLEDGE_QUESTION_LENGTH} 个字符。` }, { status: 400 }));
  }
  ...
  try {
    ...
    if (!(await consumeWriteRateLimit(db, { actorSubject: authorized.accountUserId, scope: "lab_ai_ask", limit: MAX_QUESTIONS_PER_MINUTE }))) {
      return finish(privateJson({ error: "提问过于频繁，请稍后再试。" }, { status: 429, headers: { "retry-after": "60" } }));
    }
    ...
    const ranked = await measureWaiting(timings, "lookup", async () => {
      ...
      return rankKnowledgeChunks(question, candidates, 6);
    });
    const answer = await measureWaiting(timings, "answer", () => answerLabQuestion(question, ranked));
