...
async function parseMessageRequest(request: Request): Promise<{ recipientEmail: string; messageBody: string } | null> {
  const parsed = await readBoundedJsonObject(request, MAX_REQUEST_LENGTH);
  if (!parsed.ok) return null;
  const record = parsed.value;
  if (Object.keys(record).some((key) => key !== "recipientEmail" && key !== "body")) return null;
  ...
  if (!recipientEmail || recipientEmail.length > 254 || !messageBody || messageBody.length > MAX_MESSAGE_LENGTH) return null;
  return { recipientEmail, messageBody };
}
...
export async function POST(request: Request) {
  ...
  if (!parsed) return privateJson({ error: "请输入 1–1000 字的私聊内容。" }, { status: 400 });
  ...
  try {
    ...
    }).from(sql`(SELECT 1) AS authorization_source`).where(authorizedMemberGuard(authorized))).returning();
    if (!created) return privateJson({ error: "成员状态或权限刚刚发生变化，消息未发送，请刷新后重试。" }, { status: 409 });
