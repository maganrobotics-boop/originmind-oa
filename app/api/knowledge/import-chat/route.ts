...
export async function POST(request: Request) {
  ...
  if (!Object.hasOwn(parsed.value, "document") || envelopeKeys.some((key) => key !== "document" && key !== "returnedKnowledgeItemId")) return reply({ error: "导入数据包含不支持的字段。" }, 400);
  ...
  try {
    ...
    const item = existing ? await resubmitKnowledgeItem(existing, actor, submission, identity.contentHash, contentParts) : await createChatImportedKnowledgeItem(actor, submission, identity.contentHash, identity.itemId, contentParts);
    ...
    return reply({ received: true, item: { id: item.id, title: item.title, status: item.status, visibility: item.visibility, currentRevisionNo: item.currentRevisionNo }, partCount: imported.partCount, assetUpload: { revisionId: current.current_revision_id, uploadToken: knowledgeAssetUploadToken() } }, 201);
