...
const defaultConfigPath = resolve(projectRoot, ".wrangler/generated/wrangler.staging.json");
const { workerName: expectedWorkerName, databaseName: expectedDatabaseName } = deploymentTarget("staging", process.env);
...
function usage() {
  process.stderr.write(`Usage: OA_STAGING_IMPORT_CONFIRM=${expectedWorkerName} node scripts/import-migration-to-staging.mjs --input <encrypted-archive> --private-key <jwk> --auth-key-file <file> --freeze-id-file <file> --receipt <new-json-file> --expected-origin <https-origin> [--config <staging-wrangler-json>]\n`);
  process.exitCode = 64;
}
...
function stagingBinding(config) {
  if (!config || config.name !== expectedWorkerName || !ACCOUNT_ID_PATTERN.test(config.account_id || "") || config.route || config.routes) throw new Error("Staging Wrangler configuration is not the isolated authorized target");
  const bindings = Array.isArray(config.d1_databases) ? config.d1_databases.filter((entry) => entry?.binding === "DB") : [];
  if (bindings.length !== 1 || bindings[0].database_name !== expectedDatabaseName || !UUID_PATTERN.test(bindings[0].database_id || "")) throw new Error("Staging D1 binding is invalid");
  return bindings[0];
}
...
async function validateCloudflareTarget(configPath, config, binding) {
  ...
  if (identity.loggedIn !== true || !authorizedAccounts.includes(config.account_id)) throw new Error("Wrangler is not authorized for the configured staging account");
  if (database.name !== expectedDatabaseName || database.uuid !== binding.database_id) throw new Error("Cloudflare returned a different staging D1 database");
}
...
if (options) {
  ...
  if (required.some((key) => !options[key]) || Object.keys(options).some((key) => !allowed.includes(key))) {
    usage();
  } else if (process.env.OA_STAGING_IMPORT_CONFIRM !== expectedWorkerName) {
    throw new Error(`Set OA_STAGING_IMPORT_CONFIRM=${expectedWorkerName} for an intentional staging import`);
  } else {
    ...
    if (configPath !== defaultConfigPath) throw new Error("Only the generated isolated staging Wrangler configuration may be used");
    ...
    if (queryCount > MIGRATION_IMPORT_MAX_D1_QUERIES) throw new Error(`Migration needs ${queryCount} D1 queries; the staging import was not started`);
    ...
    if (!ACCOUNT_ID_PATTERN.test(authorizedAccountId) || !UUID_PATTERN.test(authorizedDatabaseId)
      ...
      throw new Error("Generated staging configuration does not match the explicitly authorized Cloudflare account and D1 database");
    }
    ...
    if (typeof recovery.bookmark !== "string" || !recovery.bookmark) throw new Error("Could not record the staging D1 recovery bookmark");

    const receipt = {
      createdAt: new Date().toISOString(),
      databaseId: binding.database_id,
      databaseName: binding.database_name,
      format: "originmind-oa-staging-import-recovery",
      freezeId,
    ...
    const temporaryDirectory = await mkdtemp(join(tmpdir(), "originmind-staging-import-"));
    let child;
    try {
      ...
      if (!response.ok || result.ok !== true) throw new Error(`Staging import stopped with state ${result.state || "unknown"}: ${result.error || "no details"}`);
      ...
      process.stdout.write(`Staging migration ${result.state}: ${result.tables.length} tables, ${totalRows} rows, ${queryCount} D1 queries.\n`);
    } finally {
