...
try {
 for(const [name,width,height] of [['desktop',1280,900],['mobile',390,844]]){
  ...
  try {
   ...
   await alice.getByRole('button',{name:'聊天更多操作'}).click();await alice.getByRole('menuitem',{name:'返回 AI 聊天'}).click();assert.ok((await alice.locator('.message.assistant').innerText()).includes('正文完整结束'));
   await alice.getByRole('button',{name:'聊天更多操作'}).click();await alice.getByRole('menuitem',{name:'清空聊天',exact:true}).click();await alice.getByText('想了解实验室的什么？',{exact:true}).waitFor();assert.equal(store.length,2,'clearing AI must not delete member messages');
