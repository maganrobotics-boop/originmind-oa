...
try {
 for(const [name,width,height] of [['desktop',1280,900],['mobile',390,844],['mobile-short',390,560]]){
  ...
  try {
   ...
   await alice.getByRole('button',{name:'聊天选项'}).click();await alice.getByRole('menuitem',{name:'清空聊天',exact:true}).click();await alice.getByText('实验室大模型能做什么',{exact:true}).waitFor();assert.equal(store.length,2,'clearing AI must not delete member messages');
