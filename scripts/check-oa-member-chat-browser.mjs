 ...
 for(const [name,width,height] of [['desktop',1280,900],['mobile',390,844]]){
  ...
  const open=async person=>{
   ...
   await page.route('**/*',async route=>{
    ...
    if(url.pathname==='/api/session') data={registered:true,status:'active',user:{email:person.email,displayName:person.name,authProvider:'github'},role:'project_owner',isAdmin:true,canReviewKnowledge:true,canReviewMembers:true,ndaCompleted:true,needsNda:false};
    else if(url.pathname==='/api/lab-ai/status')data={authorized:true,modelReady:true,knowledgeReady:true,retrievalReady:true,budgetReady:true};
     ...
     else data={conversations:people.filter(p=>p.email!==person.email).map(p=>({peer:p,latestMessageId:null,latestCreatedAt:null,latestIncomingId:null}))};
  ...
  try {
   ...
   await alice.getByRole('button',{name:'聊天选项'}).click();await alice.getByRole('menuitem',{name:'转发最近回答'}).click();await alice.getByRole('button',{name:'选择 成员乙'}).click();
   ...
   await bob.getByRole('button',{name:'聊天选项'}).click();await bob.getByRole('menuitem',{name:'与成员聊天'}).click();await bob.getByRole('button',{name:'与 成员甲 聊天'}).click();
   ...
   await alice.getByRole('button',{name:'聊天选项'}).click();await alice.getByRole('menuitem',{name:'返回 AI 聊天'}).click();await alice.locator('.oa-conversation-ai').waitFor({state:'visible'});await alice.locator('.oa-member-title').waitFor({state:'detached'});assert.ok((await alice.locator('.message.assistant').innerText()).includes('正文完整结束'));
   ...
   assert.equal(await nav.locator('.oa-sidebar-new-chat').count(),1);assert.equal(await nav.locator('.oa-sidebar-account').count(),1);await nav.getByRole('button',{name:'审批工作台',exact:true}).click();assert.equal(await alice.locator('.dashboard-ai-entry').count(),0);
   ...
   assert.deepEqual(errors,[]);results.push({name,passed:true,forwardedCharacters:preview.length,explicitConfirmation:true,currentUserSender:true,recipientReply:true,noAiOnPrivateMessages:true,noProductionData:true});
   console.log(`${name}: explicit forwarding, full content, current-user sender, reply and title switching passed`);
