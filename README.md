...
Register exact GitHub and Feishu callbacks for each origin; wildcard callback matching is not supported. Keep both flags `false` until the complete `0000`–`0027` chain and compatible application version are deployed. Disabling either flag rejects that provider's existing OAuth sessions. Feishu login validates the configured app, tenant key, OAuth state, browser nonce, PKCE transaction and explicit member binding; it does not request email and never merges members by name or email.
