...
/** Only a leading, allowlisted @mention is a command. Quoted text/materials are never scanned. */
export function resolveChatCapability(instruction) {
  if (typeof instruction !== 'string') return null;
  const normalized = instruction.trim();
  const match = /^[@＠](知识问答|资料整理|会议纪要|项目总结)/u.exec(normalized);
  if (!match) return null;
  const hint = CHAT_DOCUMENT_HINTS.find(item => item.label === match[1]);
  const request = normalized.slice(match[0].length).replace(/^[\s:：,，、]+/u, '').trim();
  const kind = hint.label === '知识问答' ? null : hint.label === '会议纪要' ? 'meeting_minutes' : 'document';
  return { label: hint.label, kind, request,
    instruction: kind === null ? request || hint.prompt : `${hint.prompt}${request ? `\n\n具体要求：${request}` : ''}` };
}

export function wantsChatDocument(instruction) {
  const capability = resolveChatCapability(instruction);
  if (capability) return capability.kind !== null;
...
export function planChatDocument(instruction, source, previousAnswer = '') {
  const capability = resolveChatCapability(instruction);
  if (capability?.kind === null) throw new Error('知识问答不生成文档，请直接发送问题。');
  const normalized = capability?.instruction || (typeof instruction === 'string' ? instruction.trim() : '');
  const material = source?.text || previousAnswer || `未提供原始资料；以下仅为用户的写作要求，事实、日期、数据和负责人缺失时须标注“待补充”，不得编造。\n\n${normalized}`;
  const title = (source?.name ? source.name.replace(/\.(txt|md|markdown|docx)$/iu, '') : capability?.label || normalized.split(/[\r\n。！？]/u)[0]).replace(/[\r\n\t\/\\:*?"<>|]/gu, '_').slice(0, 100).trim() || '整理文档';
  const kind = capability?.kind || (/会议纪要|meeting\s+minutes/iu.test(normalized) ? 'meeting_minutes'
