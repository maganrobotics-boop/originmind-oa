...
// Canonical knowledge chunks are omitted from new snapshots and reconstructed
// from authenticated revisions on import. The limit therefore applies to
// durable source data, not duplicate search-index text.
export const MIGRATION_EXPORT_MAX_PLAINTEXT_BYTES = 48 * 1024 * 1024;
// AES-GCM adds a 16-byte tag and base64url expands by at most 4/3. The small
// fixed allowance covers the wrapped key and canonical envelope fields.
export const MIGRATION_EXPORT_MAX_ENCRYPTED_BYTES = Math.ceil((MIGRATION_EXPORT_MAX_PLAINTEXT_BYTES + 16) / 3) * 4 + 16 * 1024;
...
export const MIGRATION_EXPORT_DERIVED_TABLES = Object.freeze(["knowledge_chunks"]);
...
export function migrationExportSelectSql(table) {
  ...
  const derivedFilter = MIGRATION_EXPORT_DERIVED_TABLES.includes(table.name) ? " WHERE 0" : "";
  return `SELECT ${columns} FROM ${quoteIdentifier(table.name)}${derivedFilter} ORDER BY ${orderBy}`;
}
...
export async function buildMigrationPayload({ sourceOrigin, authKey, freezeId, notBefore, notAfter, expectedSchemaSha256, batchResults, exportedAt = new Date().toISOString(), omitDerivedKnowledgeChunks = true }) {
  ...
  for (let index = 0; index < MIGRATION_EXPORT_TABLES.length; index += 1) {
    ...
    if (omitDerivedKnowledgeChunks && MIGRATION_EXPORT_DERIVED_TABLES.includes(spec.name)
      && (!result || result.success === false || !Array.isArray(result.results) || result.results.length !== 0)) {
      throw new Error(`Migration export unexpectedly read derived table ${spec.name}`);
    }
    tables.push(await migrationTableFromD1Result(spec, result));
  }

  const body = {
    ...(omitDerivedKnowledgeChunks ? { derivedTables: [...MIGRATION_EXPORT_DERIVED_TABLES] } : {}),
...
export async function verifyMigrationPayload(payload, { expectedAuthKey, expectedSourceOrigin }) {
  const legacySnapshot = payload?.derivedTables === undefined;
  const hasSupportedDerivedTables = legacySnapshot
    || canonicalJson(payload.derivedTables) === canonicalJson(MIGRATION_EXPORT_DERIVED_TABLES);
  ...
  for (let index = 0; index < MIGRATION_EXPORT_TABLES.length; index += 1) {
    ...
    if (!legacySnapshot && MIGRATION_EXPORT_DERIVED_TABLES.includes(spec.name) && table.rows.length !== 0) {
      throw new Error(`Migration derived table ${spec.name} must be omitted`);
    }
