...
export const PRODUCTION_MIGRATION_NAME = "0034_lab_oa_v2_foundation.sql";
export const PRODUCTION_MIGRATION_SHA256 = "0c441e920e692ea9f0fa7f456afb703f2b116f5db37f0af56b50dfd532036663";
export const REVIEWED_PRODUCTION_MIGRATIONS = Object.freeze({
  ...
  "0033_knowledge_asset_finalization.sql": "ec457a9c1e7cd3de7a3af12f831df4ce8fe48585e684546cf9943aeb8212b0f5",
  [PRODUCTION_MIGRATION_NAME]: PRODUCTION_MIGRATION_SHA256,
});

const REVIEWED_SCHEMA_PREFIXES = Object.freeze([
  "knowledge_",
  "conversation_events",
...
function validateReviewedMigrationSql(migrationSqlByName) {
  ...
  for (const [name, expectedDigest] of Object.entries(REVIEWED_PRODUCTION_MIGRATIONS)) {
...
export function validateProductionMigrationManifest({ migrationNames: names, migrationSqlByName }) {
  ...
  const expectedMigrationCount = Number(PRODUCTION_MIGRATION_NAME.slice(0, 4)) + 1;
  if (
...
function isReviewedSchemaObject(objectName) {
  return REVIEWED_SCHEMA_PREFIXES.some((prefix) => objectName.startsWith(prefix));
}

export function productionSchemaDefinitions(migrationSqlByName, names = Object.keys(REVIEWED_PRODUCTION_MIGRATIONS)) {
  ...
  if (!names.length || !sameStrings(names, reviewedNames.slice(0, names.length))) {
    throw new Error("Production schema must use an exact ordered prefix of the reviewed migrations");
  }
  ...
  // Reviewed migrations are a suffix of the full ledger, so model only the
  // prerequisite columns used by additive seed/backfill statements.
  ...
  try {
    ...
    const rows = database.prepare("SELECT type, name, sql FROM sqlite_master ORDER BY type, name").all()
      .filter((row) => isReviewedSchemaObject(row.name));
...
export function validateProductionMigrationState({
  ...
  const expectedMigrationCount = Number(PRODUCTION_MIGRATION_NAME.slice(0, 4)) + 1;
  if (
    expectedNames.length !== expectedMigrationCount
  ...
  const assertDefinitions = (definitionsValue, expectedObjectsValue, label) => {
    ...
    const reviewedObjects = actualObjects.filter((name) => isReviewedSchemaObject(name.slice(name.indexOf(":") + 1)));
    if (!sameStrings(actualObjects, expectedObjectsValue)) throw new Error(`Production ${label} schema does not match the release`);
    if (!sameStrings(Object.keys(definitions).sort(), reviewedObjects)) {
      throw new Error(`Expected production schema definitions do not match ${label}`);
    }
    for (const row of schemaRows) {
      const key = `${row.type}:${row.name}`;
      if (!reviewedObjects.includes(key)) continue;
    ...
    for (let lastMigration = 25; lastMigration < expectedNames.length - 1; lastMigration += 1) {
      ...
      if (lastMigration === 25) {
        ...
        const definitions = exactObject(
          exactObject(schemaDefinitionsByMigration, "Production schema prefixes")[lastMigration],
          `Migration ${lastMigration} production schema definitions`,
        );
  ...
  assertDefinitions(expectedSchemaDefinitions, expectedObjects, `migration ${PRODUCTION_MIGRATION_NAME.slice(0, 4)}`);
  return "applied";
}
