...
export const PRODUCTION_MIGRATION_NAME = "0030_large_knowledge_revision_parts.sql";
export const PRODUCTION_MIGRATION_SHA256 = "7780dc59ced3b6bc07cc8f7c07e6d20af52ddd88fc61e3e76d8ac6e0c3f8c113";
...
export function productionKnowledgeDefinitions(migrationSqlByName, names = Object.keys(REVIEWED_KNOWLEDGE_MIGRATIONS)) {
  ...
  for (const name of names) {
    ...
    for (const statement of sqlByName[name].split(/--> statement-breakpoint\s*/u).map((value) => value.trim()).filter(Boolean)) {
      ...
      const dropTrigger = normalizedStatement.match(/^DROP TRIGGER `(knowledge_[^`]+)`$/iu);
      if (dropTrigger) {
        const key = `trigger:${dropTrigger[1]}`;
        if (!definitions[key]) throw new Error(`Reviewed migration cannot replace missing schema definition ${key}`);
  ...
  const expectedCount = names.includes("0030_large_knowledge_revision_parts.sql")
    ? 51
    : names.includes("0028_needy_microchip.sql") ? 42
...
export function validateProductionMigrationState({
  ...
  if (phase === "before" && sameStrings(appliedNames, expectedNames.slice(0, -5))) {
    ...
    return "pending-0026-0027-0028-0029-0030";
  }

  if (phase === "before" && sameStrings(appliedNames, expectedNames.slice(0, -4))) {
    ...
    return "pending-0027-0028-0029-0030";
  }
  ...
  if (phase === "before" && sameStrings(appliedNames, expectedNames.slice(0, -3))) {
    ...
    return "pending-0028-0029-0030";
  }

  if (phase === "before" && sameStrings(appliedNames, expectedNames.slice(0, -2))) {
    const migration28Definitions = exactObject(migration28KnowledgeDefinitions, "Migration 0028 knowledge schema definitions");
    const migration28Objects = [...notificationObjects, ...Object.keys(migration28Definitions)].sort();
    assertDefinitions(migration28Definitions, migration28Objects, "migration 0028");
    return "pending-0029-0030";
  }

  if (phase === "before" && sameStrings(appliedNames, expectedNames.slice(0, -1))) {
    const migration29Definitions = exactObject(migration29KnowledgeDefinitions, "Migration 0029 knowledge schema definitions");
    const migration29Objects = [...notificationObjects, ...Object.keys(migration29Definitions)].sort();
    assertDefinitions(migration29Definitions, migration29Objects, "migration 0029");
    return "pending-0030";
  }

  if (!sameStrings(appliedNames, expectedNames)) {
    throw new Error(`Production migration ledger must end exactly at ${phase === "before" ? "0025, 0026, 0027, 0028, 0029, or 0030" : "0030"}`);
  }
  assertDefinitions(expectedKnowledgeDefinitions, expectedObjects, "migration 0030");
  return "applied";
}
