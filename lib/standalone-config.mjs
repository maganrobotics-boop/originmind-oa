...
// Must match WORKER_NAME in chat-cloudflare/scripts/release-support.mjs.
// Production only: isolated staging must not acquire this capability.
export const PRODUCTION_CHAT_SERVICE = Object.freeze({
  binding: "CHAT_SERVICE",
  service: "originmind-public-chat-production",
});

export function validateProductionChatServiceBindings(services) {
  if (!Array.isArray(services) || services.length !== 1
    || !services[0] || typeof services[0] !== "object"
    || Object.keys(services[0]).length !== 2
    || services[0].binding !== PRODUCTION_CHAT_SERVICE.binding
    || services[0].service !== PRODUCTION_CHAT_SERVICE.service) {
    throw new Error("Standalone CHAT_SERVICE binding must target the production Chat Worker");
  }
}
...
export function productionTarget(environment = process.env) {
  ...
  return Object.freeze({
    ...
    websiteDatabaseName,
    websiteDatabaseId,
    knowledgeAssetsBucketName: requiredCloudflareName(environment, "OA_PRODUCTION_KNOWLEDGE_ASSETS_BUCKET_NAME", ""),
...
export function buildStandaloneConfig(target, environment = process.env) {
  if (target === "production") {
    ...
    return {
      ...
      workers_dev: false,
      keep_vars: true,
      services: [{ ...PRODUCTION_CHAT_SERVICE }],
      ai: { binding: "AI" }, // OA private document conversion; never binds staging to production.
      ...
      r2_buckets: [{ binding: "KNOWLEDGE_ASSETS", bucket_name: production.knowledgeAssetsBucketName }],
