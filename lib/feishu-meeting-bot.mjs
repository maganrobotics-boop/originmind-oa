...
function upstreamFailure(phase, kind, attempt, upstreamStatus) {
  ...
  return new BotError(phase === 'auth' ? '无法连接飞书应用认证服务；未发送入会或退出请求。' : '飞书操作结果暂不明确。请先查看参会人列表，勿重复发送入会请求；需要时由主持人移出机器人。', 502, { phase, diagnostic, ...(upstreamStatus === undefined ? {} : { upstreamStatus }), outcomeUnknown: phase !== 'auth' });
}
...
async function probeJoinTransport(fetchImpl) {
  ...
  if (!response.headers.get('content-type')?.toLowerCase().includes('application/json')) throw upstreamFailure('probe', 'CONTENT_TYPE', 1, response.status);
  let data;
  try { data = await boundedJson(response, 262144); }
...
export async function handleBotRequest(request, options) {
  ...
  try {
    ...
    if (input.action === 'check') {
      await probeJoinTransport(fetchImpl);
      return json({ credentialsVerified: true, joinTransportVerified: true, permissionVerified: false, message: '应用凭据及飞书入会端点连接通过；诊断使用无效凭据和无效会议号，未加入会议。尚未验证入会权限、会议开关或灰度资格。' });
    }
