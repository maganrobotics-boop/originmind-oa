...
async function callApi(fetchImpl, path, body, token, phase) {
  ...
  try {
    ...
    data = await boundedJson(response, 262144);
  } catch {
    throw new BotError(phase === 'auth' ? '无法连接飞书应用认证服务；未发送入会或退出请求。' : '飞书操作结果暂不明确。请先查看参会人列表，勿重复发送入会请求；需要时由主持人移出机器人。', 502, { phase, outcomeUnknown: phase !== 'auth' });
  }
  if (!response.ok || data.code !== 0) {
    ...
    throw new BotError(error, 502, { phase, code, upstreamStatus: response.status, logId: safeLog(response.headers.get('x-tt-logid') || data.log_id), outcomeUnknown: response.status >= 500 && phase !== 'auth' });
  }
  return data;
}
export async function handleBotRequest(request, options) {
  ...
  try {
    ...
    if (input.action === 'check') return json({ credentialsVerified: true, permissionVerified: false, message: '应用凭据验证通过；尚未验证入会权限、会议开关或灰度资格。未加入会议。' });
