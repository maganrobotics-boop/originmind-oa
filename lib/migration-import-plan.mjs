...
export const MIGRATION_IMPORT_MAX_KNOWLEDGE_PART_CHARACTERS = 20_000;
...
export function* migrationImportRowBatches(table) {
  ...
  if (table.name === "knowledge_revision_parts") {
    const contentIndex = table.columns.indexOf("content");
    if (contentIndex < 0) throw new Error("Migration knowledge revision parts are missing content");
    for (const row of table.rows) {
      const content = row?.[contentIndex];
      if (typeof content !== "string" || !content.length || content.length > MIGRATION_IMPORT_MAX_KNOWLEDGE_PART_CHARACTERS) {
        throw new Error("Migration knowledge revision part exceeds the safe content size");
      }
    }
  }
