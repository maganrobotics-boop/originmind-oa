import {
  ...
  isWellFormedUnicode,
  parseImportedKnowledgeSubmission,
  splitKnowledgeStorageParts,
  type KnowledgeStoragePart,
...
export type ParsedChatKnowledgeImport = {
  documentId: string;
  submissions: [KnowledgeSubmission];
  parts: KnowledgeStoragePart[];
  partCount: number;
};

export function parseChatKnowledgeImport(body: Record<string, unknown>): ParsedChatKnowledgeImport {
  ...
  if (!UUID.test(value.id) || value.title.trim().length < 2 || value.title.length > 120
    || value.body.trim().length < 10 || value.url.length > 1_500
  ...
  const parsed = parseImportedKnowledgeSubmission({
    title: safePrefix(value.title.trim(), 100),
    category: CATEGORIES[value.category],
    content: value.body,
    sourceLabel: safePrefix(`Chat 管理导入 · ${value.updatedAt} · ${value.title.trim()}`, 160),
    sourceUrl: value.url,
  }, MAX_CHAT_KNOWLEDGE_IMPORT_BYTES);
  if (!parsed.ok) throw new Error(parsed.error);
  const parts = splitKnowledgeStorageParts(parsed.value.content);
  return {
    documentId: value.id,
    submissions: [parsed.value],
    parts,
