...
export async function submitKnowledgePackage(pkg, { fetcher = fetch, onProgress = () => {}, returnedKnowledgeItemId, adminKnowledgeItemId } = {}) {
  ...
  const payload = { document: { id: pkg.id, title: pkg.title, body: pkg.body, category: pkg.category, updatedAt: pkg.updatedAt, url: '' }, ...(returnedKnowledgeItemId ? { returnedKnowledgeItemId } : {}), ...(adminKnowledgeItemId ? { adminKnowledgeItemId } : {}) };
  ...
  if (adminKnowledgeItemId) {
    if (!result.item?.mutationRevision) fail('修改版本已经上传，但未获得启用凭据；请刷新后重试。');
    onProgress('正在切换正式知识版本…');
    const activated = await fetcher(`/api/knowledge/${encodeURIComponent(result.item.id)}`, { method: 'PATCH', credentials: 'same-origin', signal: AbortSignal.timeout(90000), headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'activate_admin_edit', mutationRevision: result.item.mutationRevision }) });
    const activatedData = await activated.json().catch(() => ({}));
    if (!activated.ok || !activatedData.item) fail(activatedData.error || '新版本未能启用；旧版本仍继续提供回答。');
    result.item = activatedData.item;
    onProgress('修改已生效，OA 与 Chat 将读取新版本。');
  } else onProgress('已完整提交 OA 待审核');
  return result;
}
