...
export async function submitKnowledgePackage(pkg, { fetcher = fetch, onProgress = () => {}, returnedKnowledgeItemId } = {}) {
  ...
  onProgress('正在提交正文…');
  const result = await request('/api/knowledge/import-chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) });
  if (!result.item?.id || !result.assetUpload?.revisionId || !result.assetUpload?.uploadToken) fail('正文可能已接收，但未获得完整图片上传会话；请重试。');
  const { revisionId, uploadToken } = result.assetUpload;
